...
class WebViews:
    ...
    def led_map(self):
        ...
        start_coords = ((float(self.max_lat)+float(self.min_lat))/2,
        ...
        for icao, arpt in self.airports.items():
            ...
            pop_url = '<a href="https://nfdc.faa.gov/nfdcApps/services/ajv5/airportDisplay.jsp?airportId='\
                    + icao + '"target="_blank">'
            popup = pop_url + "<b>" + icao + "</b><br>" +\
                icao +\
                ',&nbsp' + str(arpt.get_latitude()) +\
                "</a><br>Pin&nbspNumber&nbsp=&nbsp" +\
                str(arpt.get_led_index())+"<br><b><font size=+2 color=" +\
                loc_color+">" +\
                str(arpt.get_longitude()) + "</font></b>"
    ...
    def handle_appost_request(self):
        ...
        if request.method == "POST":
            ...
            self.writehmdata(hmdata, self.conf.get_string("filenames",
                                                        "heatmap_file"))
