...
def oledcenter(
    txt, ch, font, wndir=0, dim=dimswitch, onoff=0, pause=0
    ...
    ):  # Setup and print wind direction arrow
    ...
    time.sleep(0.3)  # pause long enough to be read
...
boldfont = ImageFont.truetype(
    "/usr/share/fonts/truetype/liberation/LiberationSerif-Bold.ttf", fontsize, fontindex
)
...
while j < iterations:
    for ch in range(numofdisplays):
        val = "Test\nOLED " + str(ch)
        oledcenter(
            val, ch, font, arrowdir, dimming, toggle
