...
oledposorder = (
    config.oledposorder
)  # Oled Wiring Position, 0 = Normally pos 0-7, 1 = Backwards pos 7-0

# create list of airports to exclusively display on the OLEDs
exclusive_list = (
    config.exclusive_list
)  # Must be in this format: ['KFLG', 'KSEZ', 'KPHX', 'KCMR', 'KINW', 'KPAN', 'KDVT', 'KGEU']
exclusive_flag = (
    config.exclusive_flag
)  # 0 = Do not use exclusive list, 1 = only use exclusive list

# Specific Variables to default data to display if Rotary Switch is not installed.
wind_numorarrow = (
    config.wind_numorarrow
)  # 0 = Display Wind direction using arrows, 1 = Display wind direction using numbers.
...
hour_to_display = (
    config.time_sw0
)  # hour_to_display #Offset in HOURS to choose which TAF to display
metar_taf_mos = (
    config.data_sw0
)  # config.metar_taf_mos    #0 = Display TAF, 1 = Display METAR, 2 = Display MOS, 3 = Heat Map
toggle_sw = (
    -1
...
prob = (
    config.prob
)  # probability threshhold in Percent to assume reported weather will be displayed on map or not.
...
usetimer = (
    config.usetimer
)  # 0 = No, 1 = Yes. Turn the timer on or off with this setting
...
oledused = (
    config.oledused
)  # 1 = Yes, 0 = No. Using a single OLED to display the highest winds and airports

# Misc Settings - Should match the values in metar-v3.py
update_interval = (
    config.update_interval
)  # Number of MINUTES between FAA updates - 15 minutes is a good compromise.
metar_age = (
    config.metar_age
)  # Metar Age in HOURS. This will pull the latest metar that has been published within the timeframe listed here.
num2display = (
    config.num2display
)  # number of highest wind airports to display. Can be as high as airports listed in airports file. 5 to 10 good number.
abovekts = (
    config.abovekts
)  # 1 = Yes, 0 = No. If "Yes" then only display high winds above value stored in 'minwinds' below.
minwinds = (
    config.max_wind_speed
)  # Value in knots to filter high winds. if abovekts is 1 then don't display winds less than this value on LCD/OLED
...
numofdisplays = (
    config.numofdisplays
)  # Number of OLED displays being used. 1 Oled minimum. With TCA9548A I2C Multiplexer, 8 can be used.
oledpause = config.oledpause  # Pause time in seconds between airport display updates
fontsize = (
    config.fontsize
)  # Size of font for OLED display. 24 works well with current font type
boldhiap = (
    config.boldhiap
)  # 1 = Yes, 0 = No. Bold the text for the airport that has the highest windspeed.
blankscr = (
    config.blankscr
)  # 1 = Yes, 0 = No. Add a blank screen between the group of airports to display.
offset = (
    config.offset
)  # Pixel offset for OLED text display vertically. Leave at 3 for current font type.
border = config.border  # 0 = no border, 1 = yes border. Either works well.
dimswitch = (
    config.dimswitch
)  # 0 = Full Bright, 1 = Low Bright, 2 = Medium Bright, if IC238 Light Sensor is NOT used.
dimmin = (
    config.dimmin
)  # Set value 0-255 for the minimum brightness (0=darker display, but not off)
dimmax = config.dimmax  # Set value 0-255 for the maximum brightness (bright display)
invert = (
    config.invert
)  # 0 = normal display, 1 = inverted display, supercedes toginv. Normal = white text on black background.
toginv = (
    config.toginv
)  # 0 = no toggle of inverted display. 1 = toggle inverted display between groups of airports
scrolldis = config.scrolldis  # 0 = Scroll display to left, 1 = scroll display to right
usewelcome = (
    config.usewelcome
)  # 0 = No, 1 = Yes. Display a welcome message on the displays?
welcome = config.welcome  # will display each time the FAA weather is updated.
displaytime = (
    config.displaytime
)  # 0 = No, 1 = Yes. Display the local and Zulu Time between hi-winds display
...
disp = Adafruit_SSD1306.SSD1306_128_64(
    rst=RST
)  # 128x64 or 128x32 - disp = Adafruit_SSD1306.SSD1306_128_32(rst=RST)
...
temp_lights_on = (
    0  # Set flag for next round if sleep timer is interrupted by button push.
)
...
def tca_select(
    channel,
):  # Used to tell the multiplexer which oled display to send data to.
...
def oledcenter(
    txt, ch, font, dir=0, dim=dimswitch, onoff=0, pause=0
):  # Center text vertically and horizontally
    tca_select(ch)  # Select the display to write to
    oleddim(
        dim
    )  # Set brightness, 0 = Full bright, 1 = medium bright, 2 = low brightdef oledcenter(txt): #Center text vertically and horizontally
    draw.rectangle(
        (0, 0, width - 1, height - 1), outline=border, fill=backcolor
    )  # blank the display
    x1, y1, x2, y2 = 0, 0, width, height  # create boundaries of display

    if (
        dir == "" or txt == "\n" or "Updated" in txt or "Calm" in txt
    ):  # Print text other than wind directions and speeds
        pass

    elif (
        "METARs" in txt or "TAF" in txt or "MOS" in txt or "Heat" in txt
    ):  # Print text other than wind directions and speeds
        pass
        ...
        draw.text(
            (96, 37), arrowdir, font=arrows, fill=fontcolor
        )  # lower right of oled
        txt = txt + "kts"
        pass
        ...
        if wnddir == "000" and wndsp == "0":
            ...
            txt = (
                ap + "\n" + wnddir + chr(176) + "@" + wndsp + "kts"
            )  #'360@21kts' layout

        elif gust > 0:
            txt = (
                ap + "\n" + wnddir + "@" + wndsp + "g" + str(gust)
            )  # Lance Blank - '360@5g12' layout
    ...
    draw.text(
        (x, y), txt, align="center", font=font, fill=fontcolor
...
def winddir(
    ...
    if (wndir >= 338 and wndir <= 360) or (
        wndir >= 1 and wndir <= 22
    ):  # 8 arrows representing 45 degrees each around the compass.
...
def oleddim(
    level=0,
):  # Dimming routine. 0 = Full Brightness, 1 = low brightness, 2 = medium brightness. See https://www.youtube.com/watch?v=hFpXfSnDNSY a$
    if (
        level == 0
    ):  # https://github.com/adafruit/Adafruit_Python_SSD1306/blob/master/Adafruit_SSD1306/SSD1306.py for more info.
...
def comp_time(taf_time):
    ...
    date1 = taf_time
    date2 = current_zulu
    diff = datetime.strptime(date1, datetimeFormat) - datetime.strptime(
    ...
    if (
        ...
        for val in temp:  # Check each item in the list
            ...
            if len(val) == 6:  # this is for T06 to build appropriate length list
                ...
                len(val) > 2 and tmp_sw == 0
            ):  # if item is 1 or 2 chars long, then bypass. Otherwise fix.
...
while True:
    ...
    zulu = datetime.utcnow() + timedelta(
        hours=hour_to_display
    )  # Get current time plus Offset
    current_zulu = zulu.strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    )  # Format time to match whats reported in TAF
    current_hr_zulu = zulu.strftime(
        "%H"
    ...
    if (
        metar_taf_mos == 1
    ):  # Check to see if the script should display TAF data (0) or METAR data (1)
        # Define URL to get weather METARS. If no METAR reported withing the last 2.5 hours, Airport LED will be white (nowx).
        url = (
            "https://www.aviationweather.gov/adds/dataserver_current/httpparam?dataSource=metars&requestType=retrieve&format=xml&mostRecentForEachStation=constraint&hoursBeforeNow="
            + str(metar_age)
            + "&stationString="
        )
        ...
        url = (
            "https://www.aviationweather.gov/adds/dataserver_current/httpparam?dataSource=tafs&requestType=retrieve&format=xml&mostRecentForEachStation=constraint&hoursBeforeNow="
            + str(metar_age)
            + "&stationString="
        )
        ...
        metar_taf_mos == 2
    ):  # MOS data. This is not accessible in the same way as METARs and TAF's.
    ...
    if metar_taf_mos != 2 and metar_taf_mos != 3:
        ...
        while (
            True
        ):  # check internet availability and retry if necessary. Power outage, map may boot quicker than router.
            try:
    ...
    if metar_taf_mos == 2:
        ...
        for line in lines:  # read the MOS data file line by line0
            ...
            if "MOS" in line:
                ...
                mos_updt_time = (
                    updt1 + " " + updt2
                )  # Grab the MOS report's update timestamp
            ...
            if ap_flag:
                ...
                if cat in categories:
                    ...
                    if cat == "HR":  # hour designation
                        ...
                        keys = list(
                            hour_dict.keys()
                        )  # Get the hours which are the keys in this dict, so they can be prope$
            ...
            if airport in mos_dict:
                ...
                for hr in keys:
                    ...
                    if int(hr) <= mos_time <= int(hr) + 2.99:
                        ...
                        wdr = (
                            mos_dict[airport][hr][1]
                        ) + "0"  # make wind direction end in zero
                        ...
                        if (
                            flightcategory != "LIFR"
                        ):  # if it's LIFR due to cloud layer, no reason to check any other things$
                ...
                if wsp == None:  # if wind speed is blank, then bypass
                    ...
                    wsp == "99"
                ):  # Check to see if the MOS data didn't report a windspeed for this airport
                    windspeedkt = 0
                else:
                    windspeedkt = int(wsp)
                ...
                if stationId in stationiddict:
                    ...
                    stationiddict[
                        stationId
                    ] = flightcategory  # build category dictionary
                ...
                if stationId in wnddirdict:
                    ...
                    wnddirdict[
                        stationId
                    ] = winddirdegree  # build wind direction dictionary
    ...
    if metar_taf_mos == 0:  # 0 equals display TAF.
        ...
        for taf in root.iter("TAF"):  # iterate through each airport's TAF
            ...
            for forecast in taf.findall(
                ...
                if (
                    taf_time_from <= current_zulu <= taf_time_to
                    ...
                    for sky_condition in forecast.findall(
                        "sky_condition"
                    ):  # for each sky_condition from the XML
                        sky_cvr = sky_condition.attrib[
                            "sky_cover"
                        ...
                        if sky_cvr in (
                            ...
                            "BKN",
                            "OVX",
                        ):  # If the layer is OVC, BKN or OVX, set Flight category based on height A$
                            ...
                            try:
                                ...
                                cld_base_ft_agl = forecast.find(
                                    "vert_vis_ft"
                                ).text  # get cloud base AGL from XML
                    ...
                    if (
                        ...
                        if (
                            forecast.find("visibility_statute_mi") is not None
                            ...
                            ):  # if Flight Category was already
                                flightcategory = "MVFR"
                    ...
                    if (
                        taf_wind_gust_kt is None or taf_wind_gust_kt == ""
                    ...
                    # grab wind direction from returned FAA data
                    if (
                        taf_wind_dir_degrees is None
                    ):  # if wind direction is blank, then bypass
                        winddirdegree = 0
                    else:
        ...
        for metar in root.iter("METAR"):
            ...
            ):  # if category is blank, then bypass
                flightcategory = "NONE"
            else:
            ...
            # grab wind speeds from returned FAA data
            if (
                metar.find("wind_speed_kt") is None
            ):  # if wind speed is blank, then bypass
                windspeedkt = 0
            else:
            ...
            # grab wind gust from returned FAA data - Lance Blank
            if (
                metar.find("wind_gust_kt") is None
            ):  # if wind speed is blank, then bypass
                windgustkt = 0
            else:
            ...
            # grab wind direction from returned FAA data
            if (
                metar.find("wind_dir_degrees") is None
            ):  # if wind speed is blank, then bypass
                winddirdegree = 0
            else:
    ...
    if exclusive_flag == 1:
        num2display = (
            config.LED_COUNT
        )  # Reset num2display to all the airports if we are using exclusive_list

    newwindsdict = dict(
        sorted(iter(windsdict.items()), key=operator.itemgetter(1), reverse=True)[
            :num2display
        ]
    )
    ...
    if (
        exclusive_flag == 1
    ):  # check if we should include an exclusive subset of airports to display
        ...
        if (
            len(tmp1) < numofdisplays
        ):  # Pad blanks if airports are less than numofdisplays
            blank = [("", "")] * (numofdisplays - len(tmp1))
            tmp1 = tmp1 + blank

        sortwindslist = tmp1  # Reset sortwindslist to only those whose winds are higher than specified

    hiap, hiwind = sortwindslist[
        0
    ]  # Get the highest wind speed airport identifier and its wind speed.

    if (
        abovekts == 1
    ):  # check if we should only display airports whose winds are higher or equal to value in minwinds
        logger.debug(sortwindslist)

        tmp1 = sorted(
            i for i in sortwindslist if i[1] != ""
        )  # filter out blank screens that would throw an error when compared to minwinds
        logger.debug(tmp1)
        tmp1 = sorted(
            i for i in tmp1 if i[1] >= minwinds
        )  # sortwindslist if i[1] >= minwinds)
        tmp1.sort(key=lambda tup: tup[1])  # sort by wind value
        tmp1.reverse()  # Reverse so list is sorted highest to lowest

        if (
            len(tmp1) <= 0
        ):  # If there are no airports with winds higher than set add comment to string
    ...
    if (
        len(sortwindslist) < numofdisplays
    ):  # Pad blanks if airports are less than numofdisplays
    ...
    if lcddisplay:
        ...
        if (
            abovekts
        ):  # check to see if we should only display airports whose winds are higher than 'minwinds'
    ...
    if oledused:
        ...
        regfont = ImageFont.truetype(
            "/usr/share/fonts/truetype/liberation/LiberationSerif-Regular.ttf",
            fontsize,
            fontindex,
        )
        arrows = ImageFont.truetype(
            "/usr/share/fonts/truetype/misc/Arrows.ttf", fontsize, fontindex
        )
        ...
        if (
            usewelcome and toggle_sw != -1
        ):  # if toggle_sw == -1 then this script just started. Suppress welcome message for now
            ...
            if oledposorder == 0:
                ...
                startnum = (
                    numofdisplays - 1
                )  # these values are for oleds wired backwards, pos 7 thru 0
                stopnum = -1
                stepnum = -1
            ...
            welcomelist = list(
                welcome.split(" ")
            )  # create a list to use to display a welcome message if desired
            if (
                displayIP
            ):  # will display the RPI's local IP address along with welcome message if desired.
                splitIP = re.sub(
                    r"^(.*?(\..*?){1})\.", r"\1\n.", str(s.getsockname()[0])
                )  # split IP into 2 lines
            ...
            blanks = [
                ""
            ] * numofdisplays  # add blanks to end of message to clean display after message
    ...
    if (
        toggle_sw != -1
    ):  # check to see if this is the first time through and bypass if it is.
        if oledused:
            clearoleddisplays()
    ...
    timeout_start = (
        time.time()
    )  # When timer hits user-defined value, go back to outer loop to update FAA Weather.
    while time.time() < timeout_start + (
            ...
            if oledused:  # Display top AP Landings list on oleds
                ...
                while j < 10:
                    for ch in range(
                        ...
                        oledcenter(
                            val, ch, font, arrowdir, dimming, toggle, 0
                        )  # send airport and winds to proper oled display
        ...
        for f, mtime in WATCHED_FILES_MTIMES:
            if getmtime(f) != mtime:
                ...
                )  #'/NeoSectional/metar-display-v4.py'
        ...
        if usetimer:  # check to see if the user wants to use a timer.
            if time_in_range(
                timeoff, end_time, datetime.now().time()
            ):  # Part of Timer Fix - Thank You to Matthew G
                ...
                while time_in_range(
                    ...
                    if (
                        sleepmsg == 1
                    ):  # Display "Sleeping" message on first oled if desired. 0 = No, 1 = Yes

                        rch = random.randint(0, numofdisplays - 1)
                        oledcenter(
                            "Sleeping", rch, font, "", 1, toggle
                        )  # send airport and winds to proper oled display
                        time.sleep(2)
                        clearoleddisplays()

                    temp_timeoff = (
                        timeoff  # store original timeoff time and restore later.
                    )
                    time.sleep(1)

                    if (
                        GPIO.input(22) == False
                    ):  # Pushbutton for Refresh. check to see if we should turn on temporarily during sleep mo$
                        ...
                        timeoff = (
                            datetime.now() + timedelta(minutes=tempsleepon)
                        ).time()
        ...
        if GPIO.input(0) == False and toggle_sw != 0:
            ...
            metar_taf_mos = (
                data_sw0  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            ...
            metar_taf_mos = (
                data_sw1  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 1. Breaking out of loop for TAF/MOS + "
                + str(time_sw1)
                + " hour"
            )
            break
            ...
            metar_taf_mos = (
                data_sw2  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 2. Breaking out of loop for TAF/MOS + "
                + str(time_sw2)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw3  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 3. Breaking out of loop for TAF/MOS + "
                + str(time_sw3)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw4  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 4. Breaking out of loop for TAF/MOS + "
                + str(time_sw4)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw5  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 5. Breaking out of loop for TAF/MOS + "
                + str(time_sw5)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw6  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 6. Breaking out of loop for TAF/MOS + "
                + str(time_sw6)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw7  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 7. Breaking out of loop for TAF/MOS + "
                + str(time_sw7)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw8  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 8. Breaking out of loop for TAF/MOS + "
                + str(time_sw8)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw9  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 9. Breaking out of loop for TAF/MOS + "
                + str(time_sw9)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw10  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 10. Breaking out of loop for TAF/MOS + "
                + str(time_sw10)
                + " hours"
            )
            break
            ...
            metar_taf_mos = (
                data_sw11  # 0 = Display TAF. 1 = Display METAR. 2 = MOS. 3 = Heat Map
            )
            logger.info(
                "Switch in position 11. Breaking out of loop for TAF/MOS + "
                + str(time_sw11)
                + " hours"
            )
            break
        ...
        if oledused and metar_taf_mos != 3 and toggle_sw != -1:
            ...
            if (
                temp == len(sortwindslist) - 1
            ):  # Check to see if display should be inverted after each group of airports
                if toginv:
                    toggle = not (toggle)
                temp = 0
            else:
            ...
            if (
                invert
            ):  # If invert is set to 1 then display black text on white background
                toggle = 1
            ...
            for ch in range(startnum, stopnum, stepnum):  # numofdisplays-1, -1, -1):
                ...
                val = (
                    ap + "\n" + str(wnd)
                )  # Provide a starting value that will get modified by oledcenter function

                if (
                    ap == hiap and boldhiap
                ):  # Highlight the airport with the highest winds in bold text.
                    font = boldfont
                else:
                    font = regfont

                oledcenter(
                    val, ch, font, dir, dimming, toggle, 0
                )  # send airport and winds to proper oled display
            ...
            if scrolldis:  # Determine if display should scroll right=1 or left=0
                sortwindslist = (
                    sortwindslist[-1:] + sortwindslist[:-1]
            ...
            if (
                k == len(sortwindslist) - 1 and displaytime
            ):  # Display current and zulu time if displaytime = 1
                now = datetime.now()
                ...
                pos = int(
                    (numofdisplays - 3) / 2
                )  # Calculate position to display the time. Needs 3 screens minimum.
                ...
                if (
                    numofdisplays % 2 == 0
                ):  # Check if we have a odd or even num of displays
                    pass
                else:
                    oledcenter(
                        "Current\nTime", pos, font, arrowdir, dimming, toggle
                    )  # send Current Time to proper oled display

                    if (
                        numofdisplays < 2
                    ):  # Check to see if there is only 1 oled and pause between screens if there is.
                        time.sleep(oledpause)

                oledcenter(
                    localtime, pos + 1, font, arrowdir, dimming, toggle
                )  # send Local Time to proper oled display
                if (
                    numofdisplays < 2
                ):  # Check to see if there is only 1 oled and pause between screens if there is.
                    time.sleep(oledpause)
                ...
                oledcenter(
                    zulutime, pos + 2, font, arrowdir, dimming, toggle
                )  # send Zulu Time to proper oled display
