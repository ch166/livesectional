...
displayused = (
    config.displayused
)  # 0 = no, 1 = yes. If no, then only the metar.py script will be run. Otherwise both scripts will be threaded.
...
def findThisProcess(process_name):
    ps = subprocess.Popen(
        "ps -eaf | grep " + process_name, shell=True, stdout=subprocess.PIPE
    )
...
while True:
    ...
    if displayused == 0:
        ...
        if ledrunning == False:
            ...
            os.system(
                "ps -ef | grep '" + path1 + "' | awk '{print $2}' | xargs sudo kill"
            )
            time.sleep(2)
    ...
    if displayused == 1:
        ...
        if displayrunning == False:
            ...
            os.system(
                "ps -ef | grep '" + path2 + "' | awk '{print $2}' | xargs sudo kill"
            )
            time.sleep(2)
