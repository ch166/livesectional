...
#        self.pin = board.D18
        ...
        self.rand = self.conf.get_string("rotaryswitch", "rand") #0 = No, 1 = Yes, Randomize the colors used in wipes
        ...
        self.apinfodict = {}                                 #Holds pin num as key and a list to include [airport id, lat, lon]
    ...
    def start(self):
        ...
        self.leds = PixelStrip(self.pixelcount, self.pin,
                freq_hz=self.freq,
                dma=self.dma,
                invert=False,
    ...
    def updateLedState(self, led_slice):
        for key in self.airport_weathercode:
            ...
            self.airport_ledstate[key] = self.getWxColor(self.airport_weathercode, led_slice)
    ...
    def setLedAirportWeather(self, airportdb, key):
        ...
        if airportdb[key].wx_category == update_airports.AirportFlightCategory.MVFR:
            self.airport_weathercode[airportcode] = self.WX_MVFR
        if airportdb[key].wx_category == update_airports.AirportFlightCategory.VFR:
            self.airport_weathercode[airportcode] = self.WX_VFR
        if airportdb[key].wx_category == update_airports.AirportFlightCategory.IFR:
            self.airport_weathercode[airportcode] = self.WX_IFR
        if airportdb[key].wx_category == update_airports.AirportFlightCategory.LIFR:
            self.airport_weathercode[airportcode] = self.WX_LIFR
        if airportdb[key].wx_category == update_airports.AirportFlightCategory.OLD:
            self.airport_weathercode[airportcode] = self.WX_OLD
        if airportdb[key].wx_category == update_airports.AirportFlightCategory.UNKNOWN:
            self.airport_weathercode[airportcode] = self.WX_UNK
        debugging.debug("Airport WX Code : %s : " % self.airport_weathercode[airportcode])
        return
...
#    Fixed bug whereby lat/lon was miscalculated for certain wipes.
    ...
    def rainbowCycle(self, led_indexations, wait=.1):
        ...
        for j in range(256*led_indexations):
            for led_index in range(self.leds.numPixels()):
                if str(led_index) in self.nullpins:        #exclude NULL and LGND pins from wipe
                    ...
                    self.leds.setPixelColor(led_index, self.wheel((int(led_index * 256 / self.leds.numPixels()) + j) & 255))
            self.leds.show()
            time.sleep(wait/100)
    ...
    def rgbtogrb_wipes(self, led_index, data, order=0):
        if str(led_index) in self.rev_rgb_grb:             #This accommodates the use of both models of LED strings on one map.
            order = not order
            debugging.debug('Reversing rgb2grb Routine Output for LED PIN ' + str(led_index))
    ...
    def circlewipe(self, minlon, minlat, maxlon, maxlat, color1, color2):
        ...
        led_index = int(self.sizelat/rad_inc) #attempt to figure number of led_indexations necessary to cover whole map
    ...
    def radarwipe(self, centerlat,centerlon,led_index,color1,color2,sweepwidth=175,radius=50,angleinc=.05):
        PI = 3.141592653
        angle = 0
    ...
    def squarewipe(self, minlon, minlat, maxlon, maxlat, led_index, color1, color2, step=.5, wait_mult=10):
        declon = minlon
        declat = minlat
        inclon = maxlon
        inclat = maxlat
    ...
    def checkerwipe(self, minlon, minlat, maxlon, maxlat, led_index, color1, color2, cwccw=0, wait_mult=100):
        centlon = (self.center(maxlon,minlon))
        centlat = (self.center(maxlat,minlat))
        ...
        for j in range(led_index):
            for box in squarelist:
                for key in self.apinfodict:
                    ...
                    if self.findpoint(*box, px1, py1): #Asterick allows unpacking of list in function call.
                        color = self.rgbtogrb_wipes(led_index, color1, self.rgb_grb)
                    else:
                        color = self.rgbtogrb_wipes(led_index, color2, self.rgb_grb)
    ...
    def rabbit(self, color1, color2, delay):
        ...
        for led_index in range(self.leds.numPixels()):    #turn LED's on
            ...
            if str(led_index) in self.nullpins or str(rabbit) in self.nullpins: #exclude NULL and LGND pins from wipe
                self.leds.setPixelColor(led_index, Color(0,0,0))
        ...
        for led_index in range(self.leds.numPixels(),-1,-1): #turn led's off
            ...
            if str(rabbit) in self.nullpins or str(erase_pin) in self.nullpins: #exclude NULL and LGND pins from wipe
                ...
                if erase_pin < self.leds.numPixels() and erase_pin > 0:
                    color = self.rgbtogrb_wipes(erase_pin, self.black_color, self.rgb_grb)
